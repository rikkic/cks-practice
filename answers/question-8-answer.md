...
## validation
- Default-deny NetworkPolicy exists for ingress and egress in `netpol-q8`.
- Ingress to `api` allowed only from same namespace on 8080.
- Egress allowed only to kube-dns and `203.0.113.10:443`.
- `test-client` pod confirms allowed and denied traffic as specified.
...
4) Validate using the existing `test-client` pod in `netpol-q8`.
