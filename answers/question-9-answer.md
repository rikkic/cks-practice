...
- NetworkPolicy restricts egress from `finance` to DNS and internal cluster only.
- External egress to `198.51.100.20` is blocked.

## solution
1) Identify suspect pods:
   - `kubectl -n finance get pods -o wide`
   - Check logs or sidecar configs to confirm egress target.

2) Audit log review:
   - On control-plane: `rg 'finance' /var/log/kubernetes/audit.log | rg 'patch|update|create' | tail -n 50`
   - Extract user info for the most recent change.
   - Save a brief summary to `/home/ubuntu/answers/q9-audit.txt`.

3) Apply an egress-restricting NetworkPolicy in `finance`:
   - Default-deny egress.
